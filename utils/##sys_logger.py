...
def init_logger(log_filename):
    """由 app.py 的 @st.cache_resource 唯一触发，每次代码更新重载时新建文件"""
    ...
    if _current_log_file and os.path.exists(_current_log_file):
        try:
            ...
            for line in lines:
                ...
                if match:
                    parsed.append({"time": match.group(1), "icon": match.group(2), "module": match.group(3), "message": match.group(4)})
                else:
                    if parsed: parsed[-1]["message"] += f"<br>{line.strip()}"
            return parsed
        except Exception as e:
            return [{"time": "", "icon": "❌", "module": "Sys", "message": f"日志解析失败: {e}"}]
    return [{"time": "", "icon": "⚠️", "module": "Sys", "message": "暂无日志文件记录。"}]