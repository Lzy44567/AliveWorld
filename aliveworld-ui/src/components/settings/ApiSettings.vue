...
<template>
  <section>
    ...
    <div class="space-y-3">
      ...
      <div><label class="text-xs text-slate-400 block mb-1">API Key（密钥）</label><input type="password" v-model="configStore.globalSettings.apiKey" :placeholder="configStore.globalSettings.apiKeyConfigured ? '已安全保存；留空保持原密钥' : '输入 API Key'" class="w-full bg-slate-900 border border-slate-700 rounded p-2 text-sm text-slate-200 outline-none" /><p v-if="configStore.globalSettings.apiKeyConfigured" class="mt-1 text-[10px] text-emerald-500">后端已配置密钥，浏览器不会读取原文。</p></div>
    ...
    <details class="mt-5 rounded-xl border border-fuchsia-900/50 bg-fuchsia-950/10 p-3">
      ...
      <div class="mt-3 space-y-3">
        ...
        <div><label class="text-xs text-slate-400 block mb-1">API Key</label><input type="password" v-model="configStore.globalSettings.preferenceApiKey" :placeholder="configStore.globalSettings.preferenceApiKeyConfigured ? '已安全保存；留空保持原密钥' : '留空继承正文 API Key'" class="w-full bg-slate-900 border border-slate-700 rounded p-2 text-sm text-slate-200 outline-none" /></div>
