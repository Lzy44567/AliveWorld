...
const submit = async (direct = false) => {
  ...
  try {
    ...
    else await imageStore.compileAndCreate(gameStore.sessionId, task, {
...
<template>
  <div class="fixed inset-0 z-[80] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
    <div class="w-full max-w-xl rounded-2xl border border-fuchsia-800/70 bg-aw_panel shadow-2xl">
      <header class="flex items-center justify-between border-b border-slate-700 p-4"><div><h2 class="font-bold text-fuchsia-300">🎨 生成角色立绘</h2><p class="text-[11px] text-slate-500">{{ context.characterName }} · AI 会按可长期复用的角色立绘整理提示词</p></div><button @click="close" class="text-xl text-slate-400">✕</button></header>
