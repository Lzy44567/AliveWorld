...
<template>
  <div class="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
    <div class="w-full max-w-2xl rounded-2xl border border-cyan-800 bg-slate-950 p-5 shadow-2xl">
      ...
      <div v-if="status.runtime_available===false" class="mt-3 rounded border border-amber-800 bg-amber-950/30 p-2 text-xs text-amber-300">当前环境未安装可选语义依赖。重新运行 install_windows.bat 并选择 Y。</div>
      <div class="mt-5 flex flex-wrap gap-2"><button v-if="status.state!=='downloading' && !status.downloaded" @click="download" :disabled="busy || status.runtime_available===false" class="rounded bg-cyan-700 px-3 py-2 text-xs font-bold text-white disabled:opacity-40">{{ status.state==='paused'?'继续下载':'开始下载' }}</button><button v-if="status.state==='downloading'" @click="pause" class="rounded bg-amber-700 px-3 py-2 text-xs font-bold text-white">停止并保留断点</button><button v-if="status.downloaded" @click="toggle" class="rounded px-3 py-2 text-xs font-bold text-white" :class="status.enabled?'bg-slate-700':'bg-emerald-700'">{{ status.enabled?'关闭语义检索':'启用语义检索' }}</button><button v-if="status.downloaded || status.downloaded_bytes" @click="uninstall" class="rounded bg-rose-900 px-3 py-2 text-xs font-bold text-rose-200">卸载模型</button></div>
    </div>
  </div>
</template>
