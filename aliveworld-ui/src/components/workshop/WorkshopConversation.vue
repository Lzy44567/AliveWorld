  ...
  <main class="flex min-w-0 flex-1 flex-col bg-slate-950/55">
    <template v-if="workshopStore.workshopId">
      <header class="border-b border-slate-800 bg-slate-950/75 px-5 py-3">
        ...
        <div class="mt-3 flex flex-wrap gap-2"><button v-for="item in workshopStore.modes" :key="item.id" class="rounded px-3 py-1 text-[10px]" :class="workshopStore.mode===item.id?'bg-cyan-800 text-white':'bg-slate-900 text-slate-500'" @click="workshopStore.mode=item.id">{{ item.label }}</button></div>
      </header>
      ...
      <section class="min-h-0 flex-1 space-y-3 overflow-y-auto p-5 custom-scrollbar">
        ...
        <section v-if="workshopStore.proposed.length" class="rounded-xl border border-cyan-700/60 bg-cyan-950/25 p-3">
          ...
          <div v-for="operation in workshopStore.proposed" :key="operation.operation_id" class="mt-2 rounded-lg bg-black/25 p-2 text-xs text-slate-300"><strong class="text-cyan-200">{{ operationLabel(operation) }}</strong> · {{ operationTarget(operation) }}<p v-if="operation.reason" class="mt-1 text-slate-500">{{ operation.reason }}</p></div>
        </section>
        <section v-if="workshopStore.pending.length" class="rounded-xl border border-amber-700/60 bg-amber-950/25 p-3">
          ...
          <div v-for="operation in workshopStore.pending" :key="operation.operation_id" class="mt-2 rounded-lg bg-black/25 p-3 text-xs text-slate-300"><strong>{{ operationLabel(operation) }}</strong> · {{ operationTarget(operation) }}<div class="mt-2 flex gap-2"><button class="rounded bg-emerald-700 px-2 py-1" @click="workshopStore.decide(operation.operation_id,true)">接受</button><button class="rounded bg-slate-700 px-2 py-1" @click="workshopStore.decide(operation.operation_id,false)">拒绝</button></div></div>
        </section>
      </section>
