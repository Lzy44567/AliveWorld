...
class AIEngine:
    ...
    def generate_worldbook(self, inspiration):
        try:
            res = self.client.chat.completions.create(model=self.model, messages=[{"role": "system", "content": get_world_architect_prompt()}, {"role": "user", "content": f"世界灵感：{inspiration}"}], temperature=0.8)
    ...
    def get_world_reactions(self, context, player_action, dynamic_state, character_info, world_info):
        ...
        try:
            res = self.client.chat.completions.create(model=self.model, messages=[{"role": "system", "content": get_reaction_prompt(character_info, world_info)}, {"role": "user", "content": prompt}], temperature=0.7)
            raw_str = res.choices[0].message.content
            parsed = robust_json_parse(raw_str)
            return parsed.get('reactions', []), raw_str
        except Exception as e: log.error(f"推演提取失败: {e}"); return None, raw_str if 'raw_str' in locals() else str(e)
    ...
    def generate_story_and_state(self, context, player_action, chosen_reaction, dynamic_state, word_limit, character_info, style_info, world_info):
        ...
        try:
            res = self.client.chat.completions.create(model=self.model, messages=[{"role": "system", "content": get_settlement_prompt(word_limit, character_info, style_info, world_info)}, {"role": "user", "content": prompt}], temperature=0.8, max_tokens=max(800, int(word_limit * 2)))
            raw_str = res.choices[0].message.content
            return robust_json_parse(raw_str), raw_str
        except Exception as e: log.error(f"结算提取失败: {e}"); return None, raw_str if 'raw_str' in locals() else str(e)