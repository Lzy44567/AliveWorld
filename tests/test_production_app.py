...
class ProductionAppTests(unittest.TestCase):
    ...
    def test_one_port_serves_health_assets_and_spa_routes(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(health.headers.get("cache-control"), "no-store, max-age=0")
            root = client.get("/")
            workshop = client.get("/workshop/worldbook")
            self.assertIn("aliveworld", root.text)
            self.assertIn("aliveworld", workshop.text)
            self.assertEqual(root.headers.get("cache-control"), "no-store, max-age=0")
            self.assertEqual(workshop.headers.get("cache-control"), "no-store, max-age=0")
